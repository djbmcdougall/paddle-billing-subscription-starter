...
export default function ProfileShareDialog({ open, onOpenChange, userId, userName }: ProfileShareDialogProps) {
  const [copied, setCopied] = useState(false)
  const [isSharing, setIsSharing] = useState<string | null>(null)
  const { toast } = useToast()

  // Generate share URL
  const shareUrl = `https://murmur.app/profile/${userId}`

  // Generate share text
  const shareText = `Check out ${userName}'s profile on Murmur`
    ...
    try {
      ...
      switch (platform) {
        ...
        case "twitter":
          ...
          break
        case "email":
          shareLink = `mailto:?subject=Check out this profile on Murmur&body=${encodeURIComponent(`${shareText} ${shareUrl}`)}`
          break
      }
      ...
      toast({
        ...
        description: `Profile shared to ${platform}`,
        variant: "success",
      })
    } catch (error) {
      toast({
